...
class EmailUpdateVerificationNotification extends VerifyEmail
{
	/**
	...
	protected function verificationUrl($notifiable)
	{
		return URL::temporarySignedRoute(
			$this->routeName . '.email.update.verification.verify',
	...
	{
		$prefix = config('app.frontend_url') . '/' . $this->routeName . '/account/email/email-verification?url=';
		$verificationUrl = $this->verificationUrl($notifiable);

		return (new MailMessage)
			->subject(__('email-verification-update-subject'))
			->greeting(__('email-greeting', ['name' => $notifiable->name, "app_name" => config('app.name')]))
			->line(__('email-verification-update-click-link'))
